	...
	return (
		<section className="w-full 2xl:max-w-[90rem] flex justify-center px-5 md:px-10 pt-20">
			<div className="flex flex-col gap-10 w-max">
				<p className="text-2xl md:text-4xl font-semibold">You might also like</p>
				<div className="flex flex-wrap gap-10">
					{relatedBlogs.map(({ id, slug, thumbnailUrl, updatedAt, title, content, authorName }) => (
						<Link
							key={id}
						...
						>
							<div className="relative">
								...
								<p className="opacity-0 group-hover:opacity-100 absolute bottom-10 group-hover:bottom-5 left-10 group-hover:left-5 bg-white text-xs capitalize px-3 py-2 rounded-2xl w-max duration-300">{authorName}</p>
								<Image src={thumbnailUrl} width={1920} height={1080} className="w-[350px] h-[200px] rounded-2xl" alt="thumbnail" />
							</div>
